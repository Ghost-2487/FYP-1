...
print("="*70)
print("BIDIRECTIONAL LONG SHORT-TERM MEMORY (BiLSTM)")
print("="*70)
...
# ============================================
print("\n" + "="*70)
print("STEP 3: BUILD BiLSTM MODEL")
print("="*70)

# Builds a BiLSTM model with a hybrid embedding approach.
def build_refined_bilstm(vocab_size, max_length, num_classes, embedding_matrix=None,
                         lstm_units=256, dropout_rate=0.4, learning_rate=1e-3):
    ...
    # Higher Capacity BiLSTM
    x = Bidirectional(LSTM(units=lstm_units, dropout=0.1, return_sequences=False))(x)
...
print("✅ Hybrid BiLSTM built successfully!")
model.summary()
...
plt.legend()
plt.savefig(SCRIPT_DIR / "bilstm_training_history.png", dpi=150, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig(SCRIPT_DIR / "bilstm_confusion_matrix.png", dpi=150, bbox_inches='tight')
plt.show()
...
plt.title('ROC Curve — BiLSTM', fontsize=14, fontweight='bold', pad=20)
plt.legend(loc='lower right', fontsize=11)
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig(SCRIPT_DIR / "bilstm_roc_curve.png", dpi=150, bbox_inches='tight')
plt.show()
...
plt.title('Precision-Recall Curve — BiLSTM', fontsize=14, fontweight='bold', pad=20)
plt.legend(loc='lower left', fontsize=11)
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig(SCRIPT_DIR / "bilstm_pr_curve.png", dpi=150, bbox_inches='tight')
plt.show()
...
def save_dashboard_json():
    ...
    with open(SCRIPT_DIR / "bilstm_sentiment140_dashboard.json", 'w') as f:
        json.dump(dashboard, f, indent=2)
    print('✅ Dashboard JSON saved → bilstm_sentiment140_dashboard.json')
